...
const SEED_MENTORS = [
  {
    ...
    rating: 4.9, reviews: 128, bio: "10+ years in AI research at MIT & Google. Passionate about nurturing the next generation of tech leaders.",
  ...
  },
  {
    ...
    rating: 4.8, reviews: 94, bio: "Cardiologist & professor. Helped 200+ students get into top medical schools.",
  ...
  },
  {
    ...
    rating: 4.7, reviews: 67, bio: "Harvard Law grad, specializing in constitutional law. Mentor for aspiring lawyers and policy makers.",
  ...
  },
  {
    ...
    rating: 4.6, reviews: 52, bio: "Aerospace engineer at NASA. Love making complex physics tangible for curious minds.",
...
const Tag = ({ text, color }) => (
  <span style={{
    background: color ? color + "22" : COLORS.tag, color: color || COLORS.accent,
...
const MentorCard = ({ mentor, onView, matchScore }) => (
  <div className="fade-up" onClick={() => onView(mentor)} style={{
...
function RegisterForm({ onRegister, onBack }) {
  ...
  const [level, setLevel] = useState(""); const [grade, setGrade] = useState("");
  const [dept, setDept] = useState(""); const [uniYear, setUniYear] = useState("");
  const [goalUni, setGoalUni] = useState(""); const [goalDept, setGoalDept] = useState("");
  const [currentUni, setCurrentUni] = useState("");
  // Mentor fields
  const [mName, setMName] = useState(""); const [expertise, setExpertise] = useState([]);
  const [availability, setAvailability] = useState(""); const [price, setPrice] = useState("");
  const [mBio, setMBio] = useState(""); const [docs, setDocs] = useState([]);
    ...
    <div style={{ maxWidth: 480, margin: "60px auto", padding: 32 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {[
          ...
          <div key={r} className="fade-up-2" onClick={() => setRole(r)} style={{
    ...
    <div style={{ maxWidth: 520, margin: "40px auto", padding: 24 }}>
      <div className="fade-up" style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 18, padding: 32 }}>
        ...
        <Field label="Bio / About You">
          <textarea value={mBio} onChange={e => setMBio(e.target.value)} placeholder="Tell students about your background and experience…"
            style={{
              width: "100%", minHeight: 90, background: COLORS.surface, border: `1px solid ${COLORS.border}`,
              borderRadius: 9, padding: "10px 14px", color: COLORS.text, fontSize: 14, resize: "vertical",
              fontFamily: "inherit",
            }} />
        </Field>
        <Field label="Expertise Areas (select all that apply)">
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {EXPERTISES.map(e => (
              <span key={e} onClick={() => toggleExpertise(e)} style={{
        ...
        <Field label="Upload Credentials / Documents">
          <label style={{
            ...
            borderRadius: 10, padding: "18px", textAlign: "center", cursor: "pointer", color: COLORS.muted, fontSize: 13,
          }}>
          ...
          {docs.length > 0 && <div style={{ marginTop: 8 }}>
...
function ContactModal({ open, onClose, mentor, user }) {
  ...
  const send = () => {
    ...
    setInput("");
    setTimeout(() => {
      setMsgs(prev => [...prev, { from: "mentor", text: "Great question! Let's schedule a session and work through this together.", time: "Now" }]);
    }, 1200);
  };
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Rate ${mentor?.name}`}>
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {[1,2,3,4,5].map(s => (
          <span key={s} onClick={() => setStars(s)} style={{
            fontSize: 30, cursor: "pointer", color: s <= stars ? COLORS.gold : COLORS.border,
      ...
      <Field label="Your Review">
        <textarea value={text} onChange={e => setText(e.target.value)}
          ...
          style={{
            width: "100%", minHeight: 110, background: COLORS.surface, border: `1px solid ${COLORS.border}`,
            borderRadius: 9, padding: "10px 14px", color: COLORS.text, fontSize: 14, fontFamily: "inherit", resize: "vertical",
          }} />
      </Field>
      <Btn onClick={() => { onSubmit({ stars, text }); onClose(); }} disabled={!text}>Submit Review ✦</Btn>
    </Modal>
  );
}
...
function MentorDetail({ mentor, user, onBack, onContact, allReviews }) {
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "28px 20px" }}>
      ...
      <ReviewModal open={showReview} onClose={() => setShowReview(false)} mentor={mentor}
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "28px 20px" }}>
      ...
      <div className="fade-up-2" style={{ display: "flex", gap: 12, marginBottom: 28 }}>
        <Input value={search} onChange={e => setSearch(e.target.value)}
          placeholder="🔍  Search by name or subject…" style={{ flex: 1 }} />
      ...
      {recommended.length > 0 && <>
        <div className="fade-up-3" style={{ marginBottom: 12 }}>
          <h2 style={{ fontFamily: "'Playfair Display'", fontSize: 22, display: "flex", alignItems: "center", gap: 8 }}>
            ✦ Recommended for You
            <span style={{ background: COLORS.accent + "22", color: COLORS.accent, fontSize: 12, fontWeight: 600, padding: "2px 10px", borderRadius: 20, border: `1px solid ${COLORS.accent}44` }}>
  ...
  return (
    <div style={{ maxWidth: 420, margin: "80px auto", padding: 24 }}>
      <div className="fade-up" style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 18, padding: 32 }}>
        ...
        {demo.map(u => (
          <div key={u.id || u.name} onClick={() => onLogin(u)} style={{
...
export default function App() {
  ...
  const handleViewMentor = (m) => { setViewMentor(m); setPage("mentor-detail"); };
  const handleContact = (m) => { setContactMentor(m); };
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: COLORS.bg }}>
        <Navbar user={user} page={page}
          setPage={(p) => { setViewMentor(null); setPage(p); }}
          onLogout={() => { setUser(null); setPage("home"); }} />

        <div style={{ paddingBottom: 60 }}>
          {page === "home" && (
            <Dashboard user={user} mentors={mentors}
          ...
          {page === "mentor-detail" && viewMentor && (
            <MentorDetail mentor={viewMentor} user={user}
              onBack={() => setPage("home")}
              onContact={handleContact}
              allReviews={reviews} />
          )}
        ...
        <ContactModal open={!!contactMentor} mentor={contactMentor}
          user={user} onClose={() => setContactMentor(null)} />
      </div>
    </>
  );
}
